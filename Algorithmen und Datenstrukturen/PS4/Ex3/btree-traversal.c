...
void inOrder(struct btree_node* r)
{
	if(r!=NULL){
		inOrder(r->left);
		printf("%d ", r->value);
...
void inOrderNext(int p, struct btree_node* r){
	if(r!=NULL){
			inOrderNext(p, r->left);
			if (p == r->value) {
				printf("%d", r->right->value);
			}
			inOrderNext(p, r->right);
		}
}

void preOrder(struct btree_node* r)
{
	if(r!=NULL){
		printf("%d ", r->value);
		preOrder(r->left);
...
void postOrder(struct btree_node* r)
{
	if(r!=NULL){
		postOrder(r->left);
		postOrder(r->right);
