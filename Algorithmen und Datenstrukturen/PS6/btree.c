...
void eulerTourVoids(btree_node* root, void (*preVisitFunc)(void), void (*inVisitFunc)(struct btree_node* root), void (*postVisitFunc)(void));
void eulerTourTraversal(btree_node* root);

void preVisit();
void inVisit(btree_node* node);
void postVisit();
...
int main(void) {
	...
	*/
	...
	eulerTourTraversal(tree->root);
	printf("\n");
	eulerTourVoids(tree->root, preVisit, inVisit, postVisit);
...
void eulerTourVoids(btree_node* root, void (*preVisitFunc)(void), void (*inVisitFunc)(struct btree_node* root), void (*postVisitFunc)(void)){
	preVisitFunc();
	if (root->left != NULL) {
		eulerTourVoids(root->left, preVisit, inVisit, postVisit);
	}
	inVisitFunc(root);
	
	if (root->right != NULL) {
		eulerTourVoids(root->right, preVisit, inVisit, postVisit);
	}
	postVisitFunc();
}
...
void eulerTourTraversal(btree_node* root){
	...
	if (root->left != NULL) {
		eulerTourTraversal(root->left);
	}
	
	inVisit(root);
	
	if (root->right != NULL) {
		eulerTourTraversal(root->right);
	}
